...
\begin{figure}[htpb]
	...
	\caption{Eine Graph-Zuordnung von $G_1$ nach $G_2$ kann jeweils nur eine Kante von $G_1$ bezüglich des (schwachen) Fréchet-Abstands optimal zuordnen.}
...
\begin{figure}[H]
    ...
    \caption{$G_1$ ist in schwarz und $G_2$ in rot gezeichnet. Der Wurzelknoten für $G_1$, welcher die Bearbeitungsreihenfolge der Knoten für den Algorithmus sowie die Struktur
	des Suchgraphen $H$ bestimmt, ist mit $r$ gekennzeichnet.
	}
...
da diese bereits so eingebettet sind, dass sie die Punkte mit minimalem Abstand zu dem Knoten in $G_1$ haben. Für eine Knoten-Platzierung von $r$ für den dünn gezeichneten Graphen
haben wir den Fixpunkt, welcher kein Knoten von $G_2$ ist, mit einem Kreuz gekennzeichnet.
Der Fréchet-Abstand zwischen einer Kante in $G_1$ und einer ihrer Kanten-Platzierungen liegt in $\{1,4\}$.
\\
\\
Der Suchgraph $H$ besteht aus 2 gerichteten Bäumen.
\begin{figure}[H]
	\centering
	...
	\end{subfigure}
	\caption{Eine Skizze von $H$. $p_{r_1}$ kennzeichnet die $\varepsilon$-Platzierung von $r$, welche in dem dünn gezeichneten Teilgraphen von $G_2$ liegt.
	Analog kennzeichnet $p_{r_2}$ die $\varepsilon$-Platzierung von $r$, welche in dem dick gezeichneten Teilgraphen von $G_2$ liegt.}
\end{figure}
