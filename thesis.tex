...
\\
\\
Bevor wir uns nun dem letzten Schritt des Algorithmus widmen, konstruieren wir eine geeignete Suchstruktur für die Auswahl und Bewertung von Knoten-Platzierungen.
Dabei interpretieren wir $G_1$ wie im Beweis zu Lemma \ref{Lemma 1} nach wie vor als gerichteten Wurzelbaum (Out-Tree) mit beliebiger Wurzel $v_r$ und
legen einen abstrakten Graphen $H = (V_H, E_H)$ an.
\\
\\
...
Abschließend induzieren wir durch $w: E_H \to \mathbb{R}$ mit $w(i_{p_v},i_{p_u}) = \Delta(p_v,p_v)$ eine Gewichtung der Kanten von $H$.
\\
\\
...
\\
\\
Wir betrachten $G_1$ weiterhin als gerichteten Baum mit beliebiger Wurzel $v_r$.
