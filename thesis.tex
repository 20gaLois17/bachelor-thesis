...
reduziert sich die Konstruktion einer optimalen $\varepsilon$-Platzierung auf eine optimale Gesamt-Auswahl der jeweiligen Paare von Knoten-Platzierungen für jedes Paar inzidenter Knoten in $G_1$.
...
Jede (gültige) $\varepsilon$-Platzierung $p_v$ eines Knotens $v \in V_1$ wird durch einen entsprechenden Knoten $i_{p_v}$ in $H$ repräsentiert.
Für zwei Knoten-Platzierungen $p_v \in P(v)$ und $p_u \in P(u)$ enthält $H$ genau dann die gerichtete Kante $(i_{p_u},i_{p_v})$, wenn
...
\begin{equation}
	w(p_v) = \sum_{\text{$u:$ $u$ ist Nachfahre von $v$}} \min_{p_u \in P(u)} w(p_u) + w_H(p_v,p_u)
\end{equation}
...
\begin{Lem} \label{Lemma 2}
	Nachdem Schritt 4 beendet ist, gilt für den Wurzelknoten von $G_1$:
	$$w(v_r) = \text{Min-Sum}_{(w)F}(G_1,G_2)$$.
\end{Lem}
...
den bearbeiteten Knoten) optimal zugeordnet ist.

Sei $v$ ein Knoten in $G_1$, dessen Nachfahren Blätter sind. Nachdem $v$ bearbeitet wurde und wir $s$ als die Zuordnung betrachten,
die $\min{w(p_v)}: p_v \in P(v)$ realisiert, so ist der Teilgraph unter $v$ offensichlich optimal zugeordnet.
Sei nun $u$ ein Knoten in $G_1$ zu einem Iterationsschritt $i > 1$, dessen Nachfahren in dem aktualisierten Graphen $G_1$ Blätter sind.
...
\\
Die Korrektheit des Algorithmus folgt direkt aus Lemma \ref{Lemma 2}.
\\
...
\end{equation}
ergibt sich die schließlich die Laufzeitkomplexität von $O(n_1*m_2^3). \qed$
