...
Graphen, denen eine konkrete Darstellung ihrer Knoten und Kanten - zum Beispiel im euklidischen Raum - zugewiesen wird, bezeichnen wir auch als eingebettete Graphen.
\\
\\
...
bei denen ein Knoten jeweils als ein Punkt und eine zu zwei Punkten inzidente Kante jeweils als (die Punkte verbindendes) Geradenstück in der euklidischen Ebene eingebettet wird.
Geometrische Graphen eignen sich insbesondere zur Darstellung von Netzwerken wie beispielsweise Straßennetzen.
Wir wollen geometrische Graphen miteinander vergleichen, um im Anwendungsfall konkrete Aussagen über die Güte der durch den Graphen beschriebenen Repräsentation eines bestimmten Netzwerkes treffen zu können.
\\
Zum Vergleich zweier eingebetteter Graphen existieren bereits diverse Ansätze und so stößt man in der Literatur unter Anderem auf das Konzept eines Abstandsmaßes zwischen Graphen.
...
Die metrischen Eigenschaften eines Abstandsmaßes finden dabei in dieser Ausarbeitung zwar keine besondere Erwähnung, sind aber grundsätzlich ein fundamentales Betrachtungskriterium.
\\
Wir werden zunächst ein Abstandsmaß betrachten, welches erstmalig durch Aktiaya et. al \cite{Akitaya} eingeführt und untersucht wurde. Dieses hat die Besonderheit, dass neben der geometrischen Ähnlichkeit
der Graphen auf Basis ihrer konkreten Einbettungen auch ihre jeweilige (abstrakte) Konnektivtät einen bedeutenden Einfluss auf das Ausmaß ihrer Abstände nimmt.
Dafür etablieren wir initial in Kapitel \ref{Kapitel 2} die nötigen Begriffe und
motivieren anschließend die Einführung eines weiteren Abstandsmaßes, mit welchem wir uns für den Rest der Arbeit im Detail beschäftigen werden. Dieses Maß liefert uns einen weiteren Betrachtungswinkel, um weitere Aussagen
...
\\
\\
Für den schwachen Fréchet-Abstand wird die Bedingung an die Monotonie fallen gelassen. Zusätzlich können beide Kurven reparametrisiert werden.
...
\\
\\
Abbildung \ref{frechet_example} liefert ein paar Beispiele für den (schwachen) Fréchet-Abstand zwischen polygonalen Kurven, da uns dieses Szenario im weiteren Verlauf häufiger begegnen wird.
...
\begin{figure}[H]
    ...
    \caption{
	    ...
	    in den meisten Fällen nur an den Randpunkten auf (abhängig von eventueller Kolinearität und der Lage der Randpunkte zueinander).
	    ...
	    \\
	    \\
	    Quelle: In Anlehnung an Alt et. al \cite{Alt}, Seite 83, Figure 7 und eigene Darstellung.
    }
...
\begin{Def}
	...
	\begin{figure}[H]
		...
		Denn sei $W$ ein Weg auf $\{u,v\}$ von $u$ nach $v$ und $X$ ein Weg auf $G_2$ von $p_{u_1}$ nach $p_{v_2}$ (die initiale Orientierung von $W$ und $X$ ist durch jeweils einen Pfeil gekennzeichnet).
...
Seiten 9-11), liefert das Grundgerüst für die Konstruktion von $s$. Wir wollen die besagte Konstruktion zur Berechnung von Min-Sum$_{(w)F}$ motivieren
und die dafür notwendigen Berechnungsschritte im Detail beschreiben.
Der finale Algorithmus gliedert sich in insgesamt 4 Einzelschritte, die wir im Verlauf des Kapitels entsprechend kennzeichnen werden.
...
Für die Konstruktion von $s$ können entsprechend nur (schwach) gültige $\varepsilon$-Platzierungen der Knoten von $G_1$ in Frage kommen. Denn die
...
eine verbindliche Aussage über die Existenz einer (schwachen) $\varepsilon$-Platzierung von $G_1$.
...
Dazu werden wir die Knoten-Platzierungen stets \textit{greedy} - also möglichst optimal - berechnen und zuweisen.
...
Grundsätzlich ließen sich also die Eigenschaften des Min-Sum$_{dist}$ Abstands unter der Verwendung von anderen Abstandsmaßen zu betrachten.
Hier bietet sich zum Beispiel für geometrische Graphen insbesondere der
diskrete Fréchet-Abstand (siehe dazu zum Beispiel Aronov et. al \cite{Aronov}) als möglicher Kandidat an. Wir behaupten, dass wir aufgrund seiner
inherenten Eigenschaft die in $\ref{Zweite Einschränkung}$ geforderten Fixpunkte zur Gewährleistung einer polynomiellen Laufzeit der Berechnung nicht benötigen.
Eine formale Auseinandersetzung mit diesem Ansatz steht dabei allerdings noch aus.
\\
\\
Letzlich haben wir bei unserer Betrachtung des Min-Sum$_{(w)F}$ Abstands nur ein Mitglied dieser oben beschriebenen Klasse betrachtet und
